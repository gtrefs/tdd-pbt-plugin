---
name: tutorial
description: Use when a new user wants to learn TDD or PBT by doing. Triggers on phrases like "tutorial", "walk me through TDD", "teach me PBT", "I'm new to this", "show me how this works", "guided walkthrough". Runs the user through a chosen flow — TDD, PBT Safety Net, or Property-First Development — on a self-contained toy calculator problem. Delivers a narrated debrief after the hands-on phase, then hands off to the user's own project. No external files required.
---
...
- The toy problem is `Calculator.add(int a, int b)` returning `a + b`. It is
...
- The tutorial creates files only inside the user's current project directory.
  ...
  `src/test/java/tutorial/` and `src/main/java/tutorial/`), but do not invent
  a fake sandbox directory — every agent invocation writes real files.
...
On "Review the test list first": print the contents of `src/test/java/CalculatorTest.java`
...
written yet beyond an empty stub (if needed for compilation).
...
1. Call the `test-list` agent with:
   ```
   Feature: Calculator — add two integers
   Test file: src/test/java/CalculatorTest.java
   Implementation file: src/main/java/Calculator.java
...
2. After test-list completes, call `AskUserQuestion`:
   ...
   - Call the `refactor` agent:
     ...
     Refactor the implementation while keeping all tests green.
     ```
   - After Refactor completes, print:
     ```
...
Call the `find-properties` agent with:
```
Implementation file: src/main/java/Calculator.java
...
After the agent completes and reports properties found, print:
...
```json
{
  "questions": [{
    ...
    "multiSelect": false,
    "options": [
      { "label": "Implement the first property", "description": "Activate one property and see what jqwik finds." },
...
On "Review the property list first": print the contents of
`src/test/java/CalculatorProperties.java` and re-ask the checkpoint question.
...
Call the `implement-properties` agent with:
```
Properties file: src/test/java/CalculatorProperties.java
Implementation file: src/main/java/Calculator.java
...
After the agent completes and reports the result, print:
...
Call the `property-list` agent with:
```
Feature: Calculator — add two integers
Properties file: src/test/java/CalculatorProperties.java
Implementation file: src/main/java/Calculator.java
...
Call the `property-red` agent with:
```
Properties file: src/test/java/CalculatorProperties.java
...
Call the `property-green` agent with:
```
Properties file: src/test/java/CalculatorProperties.java
...
You have completed the hands-on tutorial. Here is how to carry what you
learned into your own project or kata:
