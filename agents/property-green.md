...
- If the run still failed (counterexample found), also extract the `Shrunk Sample` and
  `Original Sample` sections verbatim and append them inside the same fenced block,
  immediately after the statistics block:
...
- If the run **failed** (fast-check still finds a counterexample, and the framework ran —
  see carve-out above): the framework output block **must be the FIRST thing shown in your
